...
public class Proj1{
    ...
    public static class Map1 extends Mapper<WritableComparable, Text, Text, DoublePair> {
        ...
        public void map(WritableComparable docID, Text docContents, Context context)
        	...
        	// YOUR CODE HERE
        	HashSet gramIndex = new HashSet<Integer>();
        	int index = 0;
        	String currentWord; 

        	// find indices of all the targetWords
        	while(matcher.find()) {
        		currentWord = matcher.group().toLowerCase();
        		if (currentWord.equals(targetGram)) {
        			gramIndex.add(new Integer(index));
        		}
        		index++;
        	}
        	...
        	while(matcher.find()) {
        		currentWord = matcher.group().toLowerCase();
        		if (!currentWord.equals(targetGram)) { // case where currentWord is not target
        			if (gramIndex.isEmpty()) {
        				dVal = Double.POSITIVE_INFINITY;
        			} else {
        				Integer primitive_version;
        				dVal = Double.POSITIVE_INFINITY;
        				for (Object i : gramIndex) {
        					primitive_version = (Integer) i;
        					if (Math.abs(primitive_version.doubleValue()-index) < dVal) {
        						dVal = Math.abs(primitive_version.doubleValue() - index);
        					}
        				}
        			}
        			dVal = func.f(dVal);
        			context.write(new Text(currentWord), new DoublePair(1, dVal));
        		}
        		index++;
        	}
        }
    ...
    public static class Combine1 extends Reducer<Text, DoublePair, Text, DoublePair> {
        ...
        public void reduce(Text key, Iterable<DoublePair> values,
            ...
            for (DoublePair value: values) {
                occurences += value.getDouble1();
                dValueSum += value.getDouble2();
            }

            context.write(key, new DoublePair(occurences, dValueSum));

        }
    }


    public static class Reduce1 extends Reducer<Text, DoublePair, DoublePair, Text> {
        @Override
        public void reduce(Text key, Iterable<DoublePair> values,
            Context context) throws IOException, InterruptedException {
            ...
            context.write(new DoublePair(occurences,dValueSum), key);
        }
    }

    public static class Map2 extends Mapper<DoublePair, Text, DoublePair, Text> {
        //maybe do something, maybe don't
        // @Override
        // public void map(DoublePair key, Text values, Context context) throws IOException, InterruptedException {
        //     context.write(key, values);
        // }
    }

    public static class Reduce2 extends Reducer<DoublePair, Text, DoubleWritable, Text> {
         ...
         */
        @Override
        public void reduce(DoublePair key, Iterable<Text> values,
            Context context) throws IOException, InterruptedException {

                 // YOUR CODE HERE
            for (Text value: values) {
                context.write(new DoubleWritable(key.coRate()), value);
            }
    ...
    public static void main(String[] rawArgs) throws Exception {
...

        {
            ...
            firstJob.setOutputKeyClass(DoublePair.class);
        ...
        if(runJob2) {
            ...
            /* You may need to change things here */
            secondJob.setMapOutputKeyClass(DoublePair.class);
