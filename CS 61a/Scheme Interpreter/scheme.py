...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        local = procedure.env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body, local)
    elif isinstance(procedure, MuProcedure):
        local = env.make_call_frame(procedure.formals, args)
        return scheme_eval(procedure.body.first, local)
    else:
...
def apply_primitive(procedure, args, env):
    ...
    4
    >>> minus = env.bindings["-"]
    >>> vals = Pair(4, Pair(3, Pair(1, nil)))
    >>> apply_primitive(minus, vals, env)
    0
    """
    #convert args to python list, plist
    plist = []
    for value in args:
        plist.append(value)
...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('different number of parameters and arguments')
        i = 0
        while i <= len(formals) - 1:
...
def do_lambda_form(vals, env):
    ...
    new_vals = vals.second
    if vals.second.second == nil:
        return LambdaProcedure(formals, new_vals.first, env)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        symbol = target.first
        value = do_lambda_form(Pair(target.second, vals.second), env)
...
def do_let_form(vals, env):
    ...
    for binds in bindings:
        ...
        if len(binds) > 2:
            values = Pair(scheme_eval(Pair('begin', binds.second), env), values)
        else:
            values = Pair(scheme_eval(binds.second.first, env), values)
...
def do_or_form(vals, env):
    ...
    for i in range(0, len(vals)-1):
        if scheme_true(scheme_eval(vals[i], env)):
            return quote(scheme_eval(vals[i], env))
    return vals[len(vals)-1]
...
def do_begin_form(vals, env):
    ...
    check_form(vals, 1)
    # iterate through all but the last thing in vals, and call schem_eval on them
    for i in range(0,len(vals)-1):
        scheme_eval(vals[i], env)
    return vals[len(vals)-1] #return the last element in vals. Scheme_eval evaluates the return value
...
def check_formals(formals):
    ...
    >>> check_formals(read_line("(a 1 c)"))
    Traceback (most recent call last):
        ...
    scheme_primitives.SchemeError: 1 is not a valid scheme symbol
    >>> check_formals(read_line("(a a c)"))
    Traceback (most recent call last):
        ...
    scheme_primitives.SchemeError: formal parameter a used more than once
    """

    if not scheme_listp(formals):
        raise SchemeError('ill formed formals list')

    parameters = []
    for param in formals:
        if scheme_symbolp(param):
            if param in parameters:
                raise SchemeError('formal parameter ' + str(param) + ' used more than once')
            else:
                parameters.append(param)
        else:
            raise SchemeError(str(param) + ' is not a valid scheme symbol')
