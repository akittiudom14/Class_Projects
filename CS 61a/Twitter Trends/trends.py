...
def analyze_tweet_sentiment(tweet):
    ...
    for word in tweet_words(tweet):
        if has_sentiment(get_word_sentiment(word)):
            value += sentiment_value(get_word_sentiment(word))
            count += 1
    if count == 0:
        return make_sentiment(None)
    else: 
        return make_sentiment(value/count)
...
def find_centroid(polygon):
    ...
    (1.0, 2.0, 0.0)
    """
    # Formula for finding the area and the SUMMATION PORTIONS of x coord, and y coord respectively
    area,x,y,i =0,0,0,0
    ...
    # if Area is 0 return the first point
    if Area == 0:
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        ...
        for tweet in tweets_by_state[state]:
            sentiment_tweet = analyze_tweet_sentiment(tweet) 
            if has_sentiment(sentiment_tweet):
                sentiment += sentiment_value(sentiment_tweet)
                count += 1                          
        
        if count > 0:
            average_sentiment_value = sentiment / count
            averaged_state_sentiments[state] = average_sentiment_value
...
def group_tweets_by_hour(tweets):
    ...
    for tweet in tweets:
        time = tweet_time(tweet).hour
        if time not in tweets_by_hour:
            tweets_by_hour[time] = []
        tweets_by_hour[time].append(tweet)
